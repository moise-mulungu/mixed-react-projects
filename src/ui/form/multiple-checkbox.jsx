...
// DM: todoMM: this is implementation-specific. src/ui components should work with any page, any project - they are generic. So, adjust your Component to accept this data as a prop.(i don't know what the prop name should be, you want me to pass the initialItems as a prop?)
const initialItems = {
  anchovies: false,
  chicken: false,
  tomatoes: false,
          ...
          {itemsList.map((option) => (
            <div key={option}>
              <input
                ...
                onChange={(event) => {
                  // DM: if you need the current value, use a callback function so you can get access to prev (the previous value). In larger components what if something else called setItems and ... better: just add a tech-vocabulary item "race condition" - it is a common interview question
                  setItems((prev) => ({
                    ...prev,
