...
DM: this is too general a question to AI, and also Sider AI doesn't know you're in NextJS because you didn't tell it. So, it gave you a generic express.js solution with no NextJS-specific code. Also, let's finish up the weather app before we go forward on this. Also, look at the NextJS API routes documentation - this approach is not how it is done. No worries, but get really clear that you have to carefully scrutinize what AI tells you to do. 

DM: Also, we need to get you a moisemulungu.com domain before we can proceed with the contact email functionality. So, let's do that. I know your credit card is held up, but I believe I offered to buy that domain and then transfer it to you. 

DM: btw, if I'm working extensively with you on a particular solution, don't throw it all away and start over from scratch. At this point in your learning, it's best to discuss it with me first. I can help you get unstuck with the existing approach, which I've already done a lot of work.
...
The express package is the same as a NextJS API route. So you'll never use require('express') in this repo.
...
// DM: when this file was named contact.js, this post would call itself, resulting in an infinite loop. 
